...
const getPrediction = require('./antworte')
...
wss.on('request', function (request) {
  ...
  connection.on('message', function (message) {
    ...
    if (uname !== 'MegaBot' && utype === 'msg') {
      (async () => {
        let entity; 
        let luisAnswer = await getPrediction(umsg);
        let intent = luisAnswer.prediction.topIntent;

        /**
         * da Luis nicht immer gleiche antworten liefert, wird verucht erkannte klassen zu holen
         */
        try{

        let entity = luisAnswer.prediction.entities.Klasse[0];
        console.log(umsg+'####');
        myBot.post(intent,entity,umsg);
